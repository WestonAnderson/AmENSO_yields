...
countries = ['US']#'MX','CA','US']#['BR','AR','MX','US','CA']#,'BR']# ['AR','BR','MX','US','CA']
...
for country in countries:
    ...
    if country is 'BR':
        ...
        mon= [9,11,1]#[10,6,6] #[wheat, maize, soy], NOTE PYTHON INDEXING
        ...
        lag = ['N','N','Y'] #the -Y indicates that due to reporting, need to correlate 1980 ENSO with 1979 yield (reported for sowing)
        dataFile = 'AR_SIIA'
        stYr = [1969,1969,1969]
        endYr = [2013,2013,2013]
        mon= [9,11,10]#[10,6,6] #[wheat, maize, soy], NOTE PYTHON INDEXING
        ensoMon = [9,11,10]#[10,6,6] #this always starts at 1950, so have to use 0 for previous in US and lag as 'N'
        ...
        mon= [4,6,6] #[4,6,6]#[wheat, maize, soy], NOTE PYTHON INDEXING
        ensoMon = [4,6,6] #[4,6,6]#this always starts at 1950, so have to use 0 for previous in US and lag as 'N'
        ...
        dataFile = 'MX_SAGARPA'
        stYr = [1980,1980,1980]
        endYr = [2008,2008,2008]
        mon= [10,6,6]#[10,6,6] #[wheat, maize, soy], NOTE PYTHON INDEXING
        ensoMon = [10,6,6]#[10,6,6] #this always starts at 1950, so have to use 0 for previous in US and lag as 'N'
        breakPt = 0 #number of years in to put the breakpoint for detrending   
        crop = 'maize'
    elif country is 'CA':
        lag = ['N','N','N']
    ...
    for im in range(3):
        for isu in range(numSurrogates):
            ensoTS[:,im,isu]=surrogate(ensoAll[:,im])    
    ...
    if (crop != 'soy')&(crop != 'maize'):
        ...
        CNTRYyld = pd.DataFrame.from_csv('/Volumes/Data_Archive/Data/Yield/HistoricalData/'+dataFile+'/'+country+'_'+USwh+'_'+stORreg+'.csv',
                                      header=0)
        ...
        for STnam in CNTRYyld.columns:
            ...
            ST_temp = CNTRYyld[str(yldSt[0]):str(yldEnd[0])][STnam]
...
#                               ensoAll[np.isfinite(np.array(ST_temp)),ensoMon[0]]+1],axis=0)
            ...
            if len(ST_temp) < endYr[0]-stYr[0]-1:
                continue
            ST = np.array(signal.detrend(ST_temp,bp=breakPt)/(ST_temp-signal.detrend(ST_temp,bp=breakPt))) #%yield anom = (yld obs - yld exp)/yld exp
            for isu in range(numSurrogates): #correlate against surrogate TS to find significance
                surrogateR1.append(pearsonr(ensoTS_temp[:,isu],ST)[0])
            ...
            STr = pearsonr(ST,ensoAll_temp)[0]
            sig = pearsonr(ST,ensoAll_temp)[1]
            if printState is 'Y':
                ...
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/states/'+dataFile+'_'+STnam+'_wheat_'+str(breakPt)+lag[0]+'lag.png')
                plt.close()
                fig = plt.figure()
                plt.plot(ST_temp,'o')
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/trends/'+dataFile+'_'+STnam+'_wheat_yield'+lag[0]+'lag.png')
                plt.close()
                ST = np.append(STyrs.year[:,np.newaxis],ST.T[:,np.newaxis],axis=1)
                np.savetxt(str('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/anomData/'+dataFile+'_'+STnam+'_wheat_yield'+lag[0]+'lag.csv'),ST,delimiter=',')
            if (STr > HighBound)|(STr < LowBound): 
                states1.append(STnam)
                print str(STnam +' '+ str(sig)+' Corr:'+ str(STr))
                statesR1.append(STr)
                statesSig1.append(sig)
                REGp1.append(np.sum(CNTRYyld['2010':'2010'][STnam]* CNTRYha['2010':'2010'][STnam])/ \
                np.sum(CNTRYp['2010':'2010']['Value']))
        ...
        '/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/yieldCorr/percentAnom/wheat'+country+'.csv')
        print ' '
        print ' '
    ...
    if (crop != 'wheat')&(crop != 'soy'):
        ...
        #read in the yield data
        CNTRYyld = pd.DataFrame.from_csv('/Volumes/Data_Archive/Data/Yield/HistoricalData/'+dataFile+'/'+country+'_'+maize+'_'+stORreg+'.csv',
                                      header=0)
        CNTRYyld = CNTRYyld.sort()
        ...
        for STnam in CNTRYyld.columns:
            surrogateR1 = [] #clear previous surrogate correlations
            if np.array(CNTRYyld[str(yldSt[1]):str(yldEnd[1])][STnam]).dtype == 'O':
                continue #this skips columns with missing values
            ST_temp = CNTRYyld[str(yldSt[1]):str(yldEnd[1])][STnam]
            ...
            sig = pearsonr(ST,ensoAll_temp)[1]
            #ST = np.array(ST_temp)
            #STr = pearsonr(ST[1:]-ST[:-1],ensoAll_temp[1:]-ensoAll_temp[:-1])[0]
            #sig = pearsonr(ST[1:]-ST[:-1],ensoAll_temp[1:]-ensoAll_temp[:-1])[1]
            if printState is 'Y':
                for iy in range(sstLats.shape[0]):
                    for ix in range(sstLons.shape[1]):
                        stateSST[iy,ix]=np.corrcoef(sst_temp[:,0,iy,ix],ST)[0][1]
                ...
                m1.drawparallels(np.arange(-30,31,30),linewidth=0,labels=[0,1,0,0])
                fig.set_size_inches(16, 10)
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/states/'+dataFile+'_'+STnam+'_'+maize+'_'+str(breakPt)+lag[1]+'lag.png')
                plt.close()
                fig = plt.figure()
                plt.plot(ST_temp,'o')
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/trends/'+dataFile+'_'+STnam+'_'+maize+'_yield'+lag[1]+'lag.png')
                plt.close()
                ST = np.append(STyrs.year[:,np.newaxis],ST.T[:,np.newaxis],axis=1)
                np.savetxt(str('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/anomData/'+dataFile+'_'+STnam+'_'+maize+'_yield'+lag[1]+'lag.csv'),ST,delimiter=',')
            if (STr > HighBound)|(STr < LowBound):
                states.append(STnam)        
                print str(STnam +' '+ str(sig)+' Corr:'+ str(STr))
                statesR1.append(STr)
        ...
        '/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/yieldCorr/percentAnom/'+maize+country+'.csv')
    ...
    if (crop != 'wheat')&(crop != 'maize'):
        ...
        #read in the yield data
        CNTRYyld = pd.DataFrame.from_csv('/Volumes/Data_Archive/Data/Yield/HistoricalData/'+dataFile+'/'+country+'_soy_'+stORreg+'.csv',
                                      header=0)
        CNTRYyld = CNTRYyld.sort()
        ...
        for STnam in CNTRYyld.columns:
            surrogateR1 = [] #clear previous surrogate correlations
            if np.array(CNTRYyld[str(yldSt[2]):str(yldEnd[2])][STnam]).dtype == 'O':
                continue #this skips columns with missing values
            ST_temp = CNTRYyld[str(yldSt[2]):str(yldEnd[2])][STnam]
...
#            ST_temp_diff = ST_temp_diff.dropna()
            if len(ST_temp) < endYr[2]-stYr[2]-1:
                continue
            STyrs = ST_temp.index
            STyrs_diff = ST_temp.index.year+1
            ST = np.array(signal.detrend(ST_temp,bp=breakPt)/(ST_temp-signal.detrend(ST_temp,bp=breakPt))) #%yield anom = (yld obs - yld exp)/yld exp
...
#            print HighBound
            STr = pearsonr(ST,ensoAll_temp)[0]
#            STrfd = pearsonr(STfd,ensoAll_temp_diff)[0]
#            STrpc = pearsonr(STpc,ensoAll_temp_diff)[0]
            
            sig = pearsonr(ST,ensoAll_temp)[1]
#            sigfd = pearsonr(STfd,ensoAll_temp_diff)[1]
#            sigpc = pearsonr(STpc,ensoAll_temp_diff)[1]
            if printState is 'Y':
                for iy in range(sstLats.shape[0]):
                    for ix in range(sstLons.shape[1]):
                        stateSST[iy,ix]=np.corrcoef(sst_temp[:,0,iy,ix],ST)[0][1]
                ...
                m1.drawparallels(np.arange(-30,31,30),linewidth=0,labels=[0,1,0,0])
                fig.set_size_inches(16, 10)
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/states/'+dataFile+'_'+STnam+'_soy_'+str(breakPt)+lag[2]+'lag.png')
                plt.close()
                fig = plt.figure()
                plt.plot(ST_temp,'o')
                fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/trends/'+dataFile+'_'+STnam+'_soy_yield'+lag[2]+'lag.png')
                plt.close()
                ST = np.append(STyrs.year[:,np.newaxis],ST.T[:,np.newaxis],axis=1)
                np.savetxt(str('/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/anomData/'+dataFile+'_'+STnam+'_soy_yield'+lag[2]+'lag.csv'),ST,delimiter=',')
        ...
        '/Users/westonanderson/Desktop/Columbia/Research/Results/'+proj+'/Yields/yieldCorr/percentAnom/soy'+country+'.csv')
        print ' '
        print ' '
