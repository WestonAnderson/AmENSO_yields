...
crops = ['maize']#'winterwheat','maize','soy']#
yMin = -15; yMax = 15
ensoMon = 12 #NOT python indexing
lag = 0
notes = str('December ('+str(lag)+')')
loadNotes = '_HAwgt'
PC = 'PC1'
anomType = 'Production'
dynHA = ''
...
execfile('/Users/westonanderson/Desktop/Columbia/Research/Code/PyCode/general/helper_Functions/surrogateData.py')
#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>##<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#<>#
...
for crop in crops:
    ...
    surrogateR1 = [];surrogateR2 = [];surrogateR3 = []  #clear previous surrogate correlations
    ...
    for isu in range(numSurrogates): #correlate against surrogate TS to find significance
        ensoTS = surrogate(enso34tab.loc[str(1950+lag):str(2013+lag),str(ensoMon)])
        ensoTS1 = ensoTS[:(pcDF['year'].max()-pcDF['year'].min()+1)]
        ensoTS2 = ensoTS[:(pcPrDF['year'].max()-pcPrDF['year'].min()+1)]
        ensoTS3 = ensoTS[:(pcPrStDF['year'].max()-pcPrStDF['year'].min()+1)]
        surrogateR1.append(pearsonr(ensoTS1,pcDF['PC1'])[0])
        surrogateR2.append(pearsonr(ensoTS2,pcPrDF['pctAnom'])[0])
        surrogateR3.append(pearsonr(ensoTS3,pcPrStDF['pctAnom'])[0])
            #sort and find non parametric alphas
    surrogateR1 = np.sort(surrogateR1)
    surrogateR2 = np.sort(surrogateR2)
    surrogateR3 = np.sort(surrogateR3)
    ...
    sstPC=sstData.variables['anom'][(sstData.variables['T'][:]>=-12*(1960-(pcDF['year'].min()+lag)))&
                            (sstData.variables['T'][:]<=12*(1+pcDF['year'].max()-1960+lag)),:,
    ...
    sstPC = np.squeeze(sstPC)
    sstPC=sstPC[(np.array(range((pcDF['year'].max()+1)-pcDF['year'].min()))*12)+(ensoMon-1),...]



    sstPr=sstData.variables['anom'][(sstData.variables['T'][:]>=-12*(1960-(pcPrDF['year'].min()+lag)))&
                            (sstData.variables['T'][:]<=12*(1+pcPrDF['year'].max()-1960+lag)),:,
                            (sstData.variables['Y'][:]<latMax)&(sstData.variables['Y'][:]>latMin),
                            (sstData.variables['X'][:]<lonMax)&(sstData.variables['X'][:]>lonMin)]
    #pull only november SST anomalies
    sstPr = np.squeeze(sstPr)
    sstPr=sstPr[(np.array(range((pcPrDF['year'].max()+1)-pcPrDF['year'].min()))*12)+(ensoMon-1),...]



    sstPrSt=sstData.variables['anom'][(sstData.variables['T'][:]>=-12*(1960-(pcPrDF['year'].min()+lag)))&
                            (sstData.variables['T'][:]<=12*(1+pcPrDF['year'].max()-1960+lag)),:,
                            (sstData.variables['Y'][:]<latMax)&(sstData.variables['Y'][:]>latMin),
                            (sstData.variables['X'][:]<lonMax)&(sstData.variables['X'][:]>lonMin)]
    #pull only november SST anomalies
    sstPrSt = np.squeeze(sstPrSt)
    sstPrSt=sstPrSt[(np.array(range((pcPrStDF['year'].max()+1)-pcPrStDF['year'].min()))*12)+(ensoMon-1),...]

    for iy in range(sstLats.shape[0]):
        for ix in range(sstLons.shape[1]):
            sstPCcor[iy,ix], sstPCp[iy,ix] =pearsonr(sstPC[:,iy,ix],pcDF['PC1'])
            sstPRcor[iy,ix], sstPRp[iy,ix] =pearsonr(sstPr[:,iy,ix],pcPrDF['pctAnom'])
            sstPRSTcor[iy,ix], sstPRSTp[iy,ix] =pearsonr(sstPrSt[:,iy,ix],pcPrStDF['pctAnom'])
    ...
    fig.savefig('/Users/westonanderson/Desktop/Columbia/Research/Results/AmENSO_yields/Yields/yldLifeCycle/'+anomType+'/'+crop+'_'+yrMin2+yrMax2+'lifeCycle'+ARs+USw+notes+PC+dynHA+loadNotes+'.png')
    plt.close()
